...
const Dashboard = () => {
  ...
  const stats = {
    ...
    avgRating: books.filter(b => b.rating > 0).length
      ...
      : '—',
    pagesRead: books.reduce((a, b) =>
      a + (b.status === 'Finished' ? (b.pages || 0) : (b.currentPage || 0)), 0),
  ...
  const GOAL = 25
  const goalPct = Math.min(100, Math.round((stats.completedThisYear / GOAL) * 100))
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-10">
        <StatCard label="Total Books"    value={stats.totalBooks}       icon={BookIcon}    subValue="In your library" />
        <StatCard label="Books Read"     value={stats.booksRead}        icon={CheckCircle} colorClass="text-emerald-600 dark:text-emerald-400"
          subValue={stats.totalBooks ? `${((stats.booksRead/stats.totalBooks)*100).toFixed(0)}% of library` : '—'} />
        <StatCard label="Reading Now"    value={stats.currentlyReading} icon={Clock}       colorClass="text-blue-500 dark:text-blue-400" subValue="In progress" />
        <StatCard label="Avg Rating"     value={stats.avgRating}        icon={Star}        colorClass="text-amber-500 dark:text-amber-400" subValue="Out of 5 stars" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6 text-center">
            <h3 className="font-bold text-stone-700 dark:text-[#e8ddd3] mb-4">2026 Reading Goal</h3>
            <div className="relative w-32 h-32 mx-auto mb-4">
              <svg className="rotate-[-90deg]" viewBox="0 0 120 120">
                <circle cx="60" cy="60" r="50" fill="none" stroke="currentColor" strokeWidth="10" className="text-stone-100 dark:text-[#2a221a]" />
                <circle
                  cx="60" cy="60" r="50"
                  fill="none" stroke="#c9956a" strokeWidth="10"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-2xl font-extrabold text-stone-800 dark:text-[#e8ddd3]">{stats.completedThisYear}</span>
                <span className="text-[10px] text-stone-400 dark:text-stone-600 uppercase tracking-wider">of {GOAL}</span>
              </div>
            </div>
            <p className="text-xs text-stone-500 dark:text-stone-500">{goalPct}% toward your annual goal</p>
          </Card>

          {/* Quick stats */}
          <Card className="p-6 space-y-4">
            <h3 className="font-bold text-stone-700 dark:text-[#e8ddd3] text-sm">Library Snapshot</h3>
            {[
              { icon: Bookmark, label: 'Want to read', value: stats.wantToRead },
              { icon: Star,     label: 'Favorites',    value: stats.favorites },
              { icon: BookOpen, label: 'Pages read',   value: stats.pagesRead.toLocaleString() },
            ].map(({ icon: Icon, label, value }) => (
              <div key={label} className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-stone-500 dark:text-stone-500">
                  <Icon size={15} className="text-stone-400 dark:text-stone-600" />
                  {label}
                </div>
                <span className="font-bold text-stone-800 dark:text-[#e8ddd3] text-sm">{value}</span>
              </div>
            ))}
          </Card>
